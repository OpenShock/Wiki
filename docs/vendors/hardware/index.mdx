---
tags:
  - hardware
  - vendor
  - second-party-vendor
title: Hardware vendors
description: "Community-run list of vendors selling pre-built OpenShock hubs, shockers and 3D prints."
icon: Store
---

This is a **non-curated list** of self-reported vendors from the OpenShock **community**.

<Callout type="warn" title="Disclaimer">
  The OpenShock team does not provide **any** guarantees about the quality of products or services
  rendered.
</Callout>

## Explanation

| Term         | Meaning                                                                                                                                             |
| ------------ | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| 🌐 From      | Where it's being sent from.                                                                                                                         |
| ✈️ Ships to  | The region(s) that the vendor ships to.                                                                                                             |
| 📡 Hubs      | Whether the vendor sells pre-assembled Hubs ([ESP32 board](/hardware/boards) + [433 MHz transmitter](/hardware/transmitter)). |
| ⚡️ Shockers  | Whether the vendor sells [shockers](/hardware/shockers).                                                                                 |
| 📦 3D Prints | Whether the vendor sells 3D-printed cases (for controllers) or spacers (for shockers).                                                              |
| 🎨 Designs   | Whether the vendor sells the Official (Nullstalgia) OpenShock PCBs or Custom designs.                                                               |

## Vendor Picker

Use these filters to quickly find vendors that ship to you and offer the products you want.

<VendorPicker />

_Want to be on this list? Hit up a maintainer on [Discord](https://discord.gg/OpenShock)._
