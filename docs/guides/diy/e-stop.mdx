---
title: E-Stop
description: Add an emergency-stop button to a DIY hub with a simple pull-up or pull-down circuit.
icon: OctagonX
---

<Callout type="info">This guide mainly focuses on adding an E-Stop to a DIY Hub.</Callout>
...
_Most_ of the GPIO pins on any given ESP32 board should work for E-Stop, however some pins are reserved for special usage and will return an error if you attempt to use them during setup. If you run into that error, adjust your setup to use a different pin with a higher number.
