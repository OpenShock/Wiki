...
description: Reference for every ShockOSC avatar parameter — shock, vibrate, sound, and their variants.
icon: List
---
...
<Callout type="success" title="Avatar Update">
...
| **`ShockOsc/{Groupname}_NextIntensity`** | float | 0 (0%) - 1 (100%)   | Overrides the intensity for triggered actions. At 0 the configured intensity is used; any value above 0 (scaled to your limit settings) overrides it. Stays in effect until you set it back to 0 or change avatar — it is not cleared after a shock. |
